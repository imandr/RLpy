...
class LossLayer(Layer):
    ...
    def __init__(self, weight=None, **args):
        ...
        self.Weight = weight or self.DefaultWeight
        
    def call(self, inputs):
        loss_values_per_step = self.compute(inputs)                             # [mb]
        self.add_loss(self.Weight * tf.reduce_sum(loss_values_per_step))        # sum over all steps
        return loss_values_per_step[:,None]                                     # [mb, 1]
        
class ActorLossLayer(LossLayer):
    
    def compute(self, inputs):
        # inputs: probs, values, action_mask, values, returns
        probs, action_mask, values, returns = inputs
        #print("ActorLossLayer: values:", values.shape,"   returns:", returns.shape)
        advantages = returns[:,0] - values[:,0]
...
class CriticLossLayer(LossLayer):
    
    def compute(self, inputs):
        # inputs: probs, values, action_mask, values, returns
        values, returns = inputs
        return tf.reduce_sum((values-returns)**2, axis=-1)
        
class EntropyLossLayer(LossLayer):
    
    DefaultWeight = 0.001
    
    def compute(self, inputs):
        # inputs: probs, values, valid_mask
        probs, valid_mask = inputs
        p = tf.clip_by_value(probs, 1e-5, 1-1e-10)
        return tf.reduce_sum(p*tf.math.log(p)*valid_mask, axis=-1)
        
class InvalidActionLossLayer(LossLayer):
    ...
    def compute(self, inputs):
        # inputs: probs, values, valid_mask
        probs, valid_mask = inputs
        return tf.reduce_sum(probs*probs*(1-valid_mask), axis=-1)
...
class Brain(object):
    ...
    def __init__(self, input_shape=None, num_actions=None, model=None, optimizer=None, hidden=128, gamma=0.99, 
        ...
        if model is None:   
            model, train_model = self.default_models(input_shape, num_actions, hidden)
        self.Model = model
        
        self.Model.summary()
        
        self.TrainModel = train_model
        if optimizer is None:   optimizer = keras.optimizers.Adagrad(learning_rate=learning_rate)
        self.TrainModel.compile(optimizer, run_eagerly=True)
    ...
    def __default_models(self, input_shape, num_actions, hidden):
        observations = layers.Input(shape=input_shape, name="input")
        common1 = layers.Dense(hidden, activation="relu", name="common1")(observations)
        ...
        probs = layers.Dense(num_actions, activation="softmax", name="probs")(common)
        
        #critic1 = layers.Dense(hidden//5, name="critic1", activation="softplus")(common)
        values = layers.Dense(1, name="values")(common)

        model = keras.Model(inputs=[observations], outputs=[probs, values])
        
        action_mask = layers.Input(shape=(num_actions,), name="action_mask_input")
        valid_mask = layers.Input(shape=(num_actions,), name="valid_mask_input")
        returns = layers.Input(shape=(1,), name="returns_input")
        values_inp = layers.Input(shape=(1,), name="values_input")

        critic_loss = CriticLossLayer(self.CriticWeight)([values, returns])    # passes values through
        actor_loss = ActorLossLayer(self.ActorWeight)([probs, critic_loss, action_mask, values_inp, returns])    # passes values through
        entropy_loss = EntropyLossLayer(self.EntropyWeight)([probs, actor_loss, valid_mask])                       # passes values through
        invalid_actions_loss = InvalidActionLossLayer(self.InvalidActionWeight)([probs, entropy_loss, valid_mask])  # passes values through
        
        train_model = keras.Model([observations, values_inp, action_mask, valid_mask, returns], [invalid_actions_loss])

        return model, train_model
        
    def default_models(self, input_shape, num_actions, hidden):
        observations = layers.Input(shape=input_shape, name="input")
        common1 = layers.Dense(hidden, activation="relu", name="common1")(observations)
        common = layers.Dense(hidden//2, activation="relu", name="common")(common1)
        ...
        critic_loss = CriticLossLayer(self.CriticWeight, name="critic_loss")([values, returns])    
        actor_loss = ActorLossLayer(self.ActorWeight, name="actor_loss")([probs, action_mask, values_inp, returns])   
        entropy_loss = EntropyLossLayer(self.EntropyWeight, name="entropy_loss")([probs, valid_mask])  
        invalid_actions_loss = InvalidActionLossLayer(self.InvalidActionWeight, name="invalid_action_loss")([probs, valid_mask])  
        
        concat_losses = Concatenate(axis=-1)([critic_loss, actor_loss, entropy_loss, invalid_actions_loss])
        
        train_model = keras.Model([observations, values_inp, action_mask, valid_mask, returns], [concat_losses])
    ...
    def train_on_episode(self, h):
        ...
        probs, values = self.Model.predict_on_batch(xcolumns)
        ...
        action_mask = np.eye(self.NActions)[actions]
        if valids is None:
            valids = np.ones((len(actions), self.NActions))
        
        train_x_columns = xcolumns + [values, action_mask, valids, returns[:,None]]
        
        output = self.TrainModel.train_on_batch(train_x_columns)
        #print("train_on_batch output:", output)
        
        entropy_loss = tf.convert_to_tensor(self.TrainModel.get_layer("entropy_loss").losses[0]).numpy()
        critic_loss = tf.convert_to_tensor(self.TrainModel.get_layer("critic_loss").losses[0]).numpy()
        actor_loss = tf.convert_to_tensor(self.TrainModel.get_layer("actor_loss").losses[0]).numpy()
        invalid_action_loss = tf.convert_to_tensor(self.TrainModel.get_layer("invalid_action_loss").losses[0]).numpy()

        entropy_loss = 0.0 if self.EntropyWeight == 0.0 else entropy_loss/self.EntropyWeight
        critic_loss = 0.0 if self.CriticWeight == 0.0 else critic_loss/self.CriticWeight
        actor_loss = 0.0 if self.ActorWeight == 0.0 else actor_loss/self.ActorWeight
        invalid_action_loss = 0.0 if self.EntropyWeight == 0.0 else invalid_action_loss/self.InvalidActionWeight
        
        return dict(
            entropy=entropy_loss,
            critic=critic_loss,   
            actor=actor_loss,
            invalids=invalid_action_loss,
            sum_values = sum_values,
            sum_advantages = sum_returns-sum_values,
    ...
    def train_on_multi_episode_history(self, multi_ep_history):
        ...
        # sum of losses over all episodes
        critic_loss = actor_loss = invalid_action_loss = entropy_loss = 0.0
        
        for h in multi_ep_history:
            stats = self.train_on_episode(h)
            actor_loss += stats["actor"]
            invalid_action_loss += stats["invalids"]
            critic_loss += stats["critic"]
            entropy_loss += stats["entropy"]
            T = len(h["actions"])
            total_steps += T
        ...
        if True:
            ...
            print("    Losses per step: critic:", critic_loss/total_steps, 
                 "  actor:", actor_loss/total_steps, 
                 "  invalid:", invalid_action_loss/total_steps, 
                 "  entropy:", entropy_loss/total_steps)
            
        averages = dict(
            actor = actor_loss/total_steps,
            critic = critic_loss/total_steps,
            entropy = entropy_loss/total_steps,
            invalid_action = invalid_action_loss/total_steps,
