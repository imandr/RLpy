...
class WalkerEnv(object):
    ...
    def create_model(self, hidden):
        ...
        from gradnet.layers import Conv2D, Flatten, Concatenate, Dense
        num_actions = self.action_space.n
        sensor_shape = (self.Window, self.Window, 1)
        sensor_input = Input(shape=sensor_shape, name="sensor")
        position_input = Input(shape=(6,), name="position")
    
        conv1 = Conv2D(3, 3, 4, activation="relu")(sensor_input)
        conv2 = Conv2D(3, 3, self.Window*self.Window, activation="tanh")(conv1)
        flat = Flatten()(conv2)
        concat = Concatenate()([flat, position_input])
    
        common1 = Dense(hidden, activation="relu", name="common1")(concat)
        ...
        model = Model([sensor_input, position_input], [probs, value])
